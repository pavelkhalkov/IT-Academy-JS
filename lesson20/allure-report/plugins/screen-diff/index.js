(function () {
  const settings = allure.getPluginSettings("screen-diff", {
    diffType: "diff",
  });
  ...
  function renderImage(src) {
    return (
      ...
      `<img class="screen-diff__image" src="${src}">` +
      "</div>"
    );
  }
  ...
  function renderDiffContent(type, diffImage, actualImage, expectedImage) {
    ...
    if (type === "overlay" && expectedImage) {
      return (
        ...
        `<img class="screen-diff__image" src="${expectedImage}">` +
        '<div class="screen-diff__image-over">' +
        `<img class="screen-diff__image" src="${actualImage}">` +
        "</div>" +
        "</div>"
      );
    }
  ...
  const TestResultView = Backbone.Marionette.View.extend({
    ...
    onRender() {
      ...
      const testType = data.labels.filter(
        (label) => label.name === "testType"
      )[0];
  ...
  var ScreenDiffView = Backbone.Marionette.View.extend({
    ...
    template(data) {
      ...
      return (
        ...
        `<label><input type="radio" name="${data.radioName}" value="diff"> Show diff</label>` +
        `<label><input type="radio" name="${data.radioName}" value="overlay"> Show overlay</label>` +
        `</div>${renderDiffContent(
          data.diffType,
          data.diffImage,
          data.actualImage,
          data.expectedImage
